...
# ToDo: remove ArcLinesMap class (it is no longer necessary)
class ArcLinesMap:
    """Auxiliary class to store arc line mapping for a given slitlet.

    Parameters
    ----------
    list_solutions_wv : list of wavelength calibration solutions
        Python list containing the wavelength calibration solutions,
        computed with the method wavelength_calibration of the
        EmirSlitlet class, for all the slitlets.
    csu_bar_slit_center_array : 1d numpy array, float
        Numpy array containing the csu_bar_slit_center of the slit for
        each slitlet. The dimension of this array must be the same
        as the length of list_solutions_wv.
    json_output : string
        If not None, the arc lines mapping if saved into a json output
        file.
    json_input : string
        If list_solutions_wv=None and csu_bar_slit_center_array=None,
        the initialization of the arc lines mapping is read from an
        external json file.
    debugplot : int
        Determines whether intermediate computations and/or plots
        are displayed:
    ...
    ----------
    list_solutions_wv : list of wavelength calibration solutions
        See description in parameters.
    csu_bar_slit_center_array : 1d numpy array, float
        See description in parameters.
    num_unique_wavelengths : int
        Number of unique wavelengths.
    unique_wavelengths : 1d numpy array, strings
        Unique arc lines identified. Note that this array contains the
        string representation of the float numbers to avoid errors when
        comparing floats.
    list_poly_arc_lines : list of Polynomials
        Polynomials fitted to the location of each unique wavelength as
        a function of csu_bar_slit_center.
    nfit : 1d numpy array, floats
        Number of times that a particular wavelength appears and is
        employed to fit the polynomial.
    list_csu_bar_slit_center : list of 1d numpy arrays, floats
        X values employed in the polynomial fit for each wavelength.
    csu_bar_slit_center_min: 1d numpy array, floats
        Minimum X value employed in the polynomial fit.
    csu_bar_slit_center_max: 1d numpy array, floats
        Maximum X value employed in the polynomial fit.
    list_xpos : list of 1d numpy arrays, floats
        Y values employed in the polynomial fit for each wavelength.
    pol_deg : 1d numpy array, integers
        Polynomial degree employed in the fit of each wavelength.
    residuals_polyfit : 1d numpy array, floats
        Residuals of each polynomial fit.
    debugplot : int
        See description in parameters.
    ...
    def __init__(self,
                 list_solutions_wv=None,
                 csu_bar_slit_center_array=None,
                 json_output=None,
                 json_input=None, debugplot=0):
        """Initialize ArcLinesMap."""

        self.debugplot = debugplot

        if list_solutions_wv is None and csu_bar_slit_center_array is None:
            if json_input is None:
                raise ValueError("No json input file has been provided.")
            # read arc lines mapping from external json file
            tmp_big_dict = json.loads(open(json_input).read())
            self.num_unique_wavelengths = \
                tmp_big_dict['num_unique_wavelengths']
            self.unique_wavelengths = \
                np.zeros(self.num_unique_wavelengths, dtype='|S9')
            self.nfit = np.zeros(self.num_unique_wavelengths, dtype=np.int)
            self.list_poly_arc_lines = []
            self.residuals_polyfit = np.zeros(self.num_unique_wavelengths)
            self.pol_deg = np.zeros(self.num_unique_wavelengths, dtype=np.int)
            self.csu_bar_slit_center_min = \
                np.zeros(self.num_unique_wavelengths)
            self.csu_bar_slit_center_max = \
                np.zeros(self.num_unique_wavelengths)
            self.list_csu_bar_slit_center = []
            self.list_xpos = []
            for i in range(self.num_unique_wavelengths):
                tmp_dict = tmp_big_dict['arcline' + str(i).zfill(4)]
                self.unique_wavelengths[i] = tmp_dict['unique_wavelength']
                if tmp_dict['poly'] is None:
                    self.list_poly_arc_lines.append(None)
                else:
                    self.list_poly_arc_lines.append(
                        Polynomial(np.array(tmp_dict['poly']))
                    )
                self.pol_deg[i] = tmp_dict['pol_deg']
                self.csu_bar_slit_center_min[i] = \
                    tmp_dict['csu_bar_slit_center_min']
                self.csu_bar_slit_center_max[i] = \
                    tmp_dict['csu_bar_slit_center_max']
                self.nfit[i] = tmp_dict['nfit']
                self.residuals_polyfit[i] = tmp_dict['rms']
                self.list_csu_bar_slit_center.append(
                    np.array(tmp_dict['csu_bar_slit_center']))
                self.list_xpos.append(np.array(tmp_dict['xpos']))
        else:
            # store initial information as data members
            self.list_solutions_wv = deepcopy(list_solutions_wv)
            self.csu_bar_slit_center_array = \
                np.copy(csu_bar_slit_center_array)

            # determine all the different wavelengths identified
            # (note that, to avoid errors when comparing floats, the actual
            # wavelengths are handled as strings)
            self.unique_wavelengths = np.array([], dtype='|S9')
            for solution_wv in list_solutions_wv:
                for reference in solution_wv.reference:
                    self.unique_wavelengths = np.append(
                        self.unique_wavelengths, "{0:9.3f}".format(reference)
                    )
            self.unique_wavelengths = np.unique(self.unique_wavelengths)
            self.num_unique_wavelengths = len(self.unique_wavelengths)

            # fit polynomial to the location of each unique wavelength as a
            # function of csu_bar_slit_center
            self.list_poly_arc_lines = []
            self.nfit = np.zeros(self.num_unique_wavelengths, dtype=np.int)
            self.list_csu_bar_slit_center = []
            self.csu_bar_slit_center_min = \
            ...
            self.residuals_polyfit = np.zeros(self.num_unique_wavelengths)
            for i in range(self.num_unique_wavelengths):
                reference_char = self.unique_wavelengths[i]
                xfit_csu_bar_slit_center = np.array([])
                yfit_xpos = np.array([])
                for solution_wv, csu_bar_slit_center in \
                        zip(list_solutions_wv, csu_bar_slit_center_array):
                    for reference, xpos in zip(solution_wv.reference,
                                               solution_wv.xpos):
                        reference_char_tmp = "{0:9.3f}".format(reference)
                        if reference_char == reference_char_tmp:
                            xfit_csu_bar_slit_center = np.append(
                                xfit_csu_bar_slit_center, csu_bar_slit_center)
                            yfit_xpos = np.append(yfit_xpos, xpos)
                self.list_csu_bar_slit_center.append(
                    xfit_csu_bar_slit_center)
                self.csu_bar_slit_center_min[i] = \
                    xfit_csu_bar_slit_center.min()
                self.csu_bar_slit_center_max[i] = \
                    xfit_csu_bar_slit_center.max()
                self.list_xpos.append(yfit_xpos)
                nfit = len(xfit_csu_bar_slit_center)
                self.nfit[i] = nfit
                if nfit >= 3:
                    pol_deg = 2
                elif nfit == 2:
                    ...
                    raise ValueError("Unexpected nfit=" + str(nfit))
                self.pol_deg[i] = pol_deg
                if pol_deg > 0:
                    polfit_line, residuals_line = polfit_residuals(
                        x=xfit_csu_bar_slit_center, y=yfit_xpos, deg=pol_deg,
                        xlabel="csu_bar_slit_center (mm)",
                        ylabel="arc line location (pixel)",
                        title="Fitting arc line of wavelength " +
                              reference_char,
                        use_r=False,
                        debugplot=0
                    )
                    self.list_poly_arc_lines.append(polfit_line)
                    self.residuals_polyfit[i] = np.std(residuals_line)
                else:
                    self.list_poly_arc_lines.append(None)
                    self.residuals_polyfit[i] = None

        # display polynomial fits to all wavelengths
        if debugplot % 10 != 0:
            import matplotlib
            matplotlib.use('Qt4Agg')
            import matplotlib.pyplot as plt
            ...
            ax.set_ylabel('xpos from solution_wv\n(pixel, from 1 to NAXIS1)')
            for i in range(self.num_unique_wavelengths):
                if self.pol_deg[i] > 0:
                    xpol = np.linspace(start=self.csu_bar_slit_center_min[i],
                                       stop=self.csu_bar_slit_center_max[i],
                                       num=1000)
                    polfit_line = self.list_poly_arc_lines[i]
                    ypol = polfit_line(xpol)
                    ax.plot(xpol, ypol, 'k-')
                xfit_csu_bar_slit_center = self.list_csu_bar_slit_center[i]
                yfit_xpos = self.list_xpos[i]
                ax.plot(xfit_csu_bar_slit_center, yfit_xpos, 'o')
                ax.set_ylim([1, NAXIS1_EMIR])
            plt.title("Number of unique wavelengths: " +
                      str(len(self.unique_wavelengths)))
            pause_debugplot(debugplot, pltshow=True)

        # save arc lines mapping into external json file
        if json_output is not None:
            tmp_big_dict = dict(
                num_unique_wavelengths=self.num_unique_wavelengths
            )
            for i in range(self.num_unique_wavelengths):
                tmp_dict = {'unique_wavelength':
                                self.unique_wavelengths[i].tolist()}
                tmp_dict['csu_bar_slit_center_min'] = \
                    self.csu_bar_slit_center_min[i].tolist()
                tmp_dict['csu_bar_slit_center_max'] = \
                    self.csu_bar_slit_center_max[i].tolist()
                tmp_poly = self.list_poly_arc_lines[i]
                if tmp_poly is None:
                    tmp_dict['poly'] = None
                else:
                    tmp_dict['poly'] = tmp_poly.coef.tolist()
                tmp_dict['nfit'] = self.nfit[i].tolist()
                tmp_dict['rms'] = self.residuals_polyfit[i].tolist()
                tmp_dict['pol_deg'] = self.pol_deg[i].tolist()
                tmp_dict['csu_bar_slit_center'] = \
                    self.list_csu_bar_slit_center[i].tolist()
                tmp_dict['xpos'] = self.list_xpos[i].tolist()
                tmp_big_dict['arcline' + str(i).zfill(4)] = tmp_dict
            with open(json_output, 'w') as fstream:
                print(json.dump(tmp_big_dict, fstream, indent=4,
                                sort_keys=True))

    def __repr__(self):
        """Define printable representation of a ArcLinesMap instance."""

        output = "<ArcLinesMap instance>\n"
        output += "- Number of unique wavelengths: " + \
                  str(self.num_unique_wavelengths) + "\n"
        for i in range(self.num_unique_wavelengths):
            output += "\n"
            output += "> reference=" + self.unique_wavelengths[i]
            output += ", nfit=" + str(self.nfit[i])
            output += ", rms=" + str(self.residuals_polyfit[i])
            output += "\n"
            output += "+ polynomial degree=" + str(self.pol_deg[i]) + "\n"
            output += "+ " + str(self.list_poly_arc_lines[i]) + "\n"
            output += "+ csu_bar_slit_center....: " + \
                      str(self.list_csu_bar_slit_center[i]) + "\n"
            output += "+ csu_bar_slit_center_min: " + \
                      str(self.csu_bar_slit_center_min[i]) + "\n"
            output += "+ csu_bar_slit_center_max: " + \
                      str(self.csu_bar_slit_center_max[i]) + "\n"
            output += "+ xpos.......: " + str(self.list_xpos[i]) + "\n"

        return output

    def predict_poly_wv(self, csu_bar_slit_center,
                        deg, times_sigma_reject=None,
                        extrapolation=False):
        """Predict polynomial for arc calibration.

        Parameters
        ----------
        csu_bar_slit_center : float
            Slit location (in mm), derived from the location of the
            CSU bars (middle point between the borders of the two bars
            defining the slit).
        deg : int
            Polynomial degree. Note that if the number of predicted arc
            lines for the requested csu_bar_slit_center value is too low,
            the final polynomial degree is adjusted accordingly.
        times_sigma_reject : None or float
            If not None, this number indicates the number of times the
            robust standard deviation (around the fit) to reject points
            iteratively.
        extrapolation : bool
            If True, allow extrapolation of polynomials providing the
            expected xpos at a given csu_bar_slit_center for each
            identified arc line.

        Returns
        -------
        poly_wv : numpy Polynomial instance
            Wavelength calibration polynomial.

        """

        # interpolate the xpos location of each arc line for the
        # particular csu_bar_slit_center value (note that only
        # predictions within the expected range are considered)
        xpos_predicted = np.array([])
        wv_predicted = np.array([])
        color_symbol = np.array([])
        for i in range(self.num_unique_wavelengths):
            # check whether csu_bar_slit_center falls within the
            # interpolation range
            csu_bar_slit_center_ok = self.csu_bar_slit_center_min[i] \
                                     <= csu_bar_slit_center \
                                     <= self.csu_bar_slit_center_max[i]
            if csu_bar_slit_center_ok or extrapolation:
                polfit_line = self.list_poly_arc_lines[i]
                if polfit_line is not None:
                    xpos = polfit_line(csu_bar_slit_center)
                    if 1 <= xpos <= NAXIS1_EMIR:
                        xpos_predicted = np.append(xpos_predicted, [xpos])
                        wv_predicted = np.append(
                            wv_predicted, [float(self.unique_wavelengths[i])])
                        if csu_bar_slit_center_ok:
                            color_symbol = np.append(color_symbol, ['b'])
                        else:
                            color_symbol = np.append(color_symbol, ['r'])

        nlines = len(xpos_predicted)
        if nlines < 2:
            raise ValueError("Number of lines=" + str(nlines) +
                             " is insufficient to fit a polynomial")

        # adjust the polynomial degree to the number of points
        # available for the fit
        deg_eff = min(deg, nlines - 1)

        # reject deviant points when requested
        if deg_eff < nlines - 1 and times_sigma_reject is not None:
            # obtain approximate linear fit
            crpix1 = 1.0
            if nlines >= 5:
                # using the robust Theil-Sen method
                intercept, slope = fit_theil_sen(xpos_predicted, wv_predicted)
                cdelt1_approx = slope
                crval1_approx = intercept + slope * crpix1
            else:
                # using a normal polynomial fit
                poly_tmp = Polynomial.fit(x=xpos_predicted, y=wv_predicted,
                                          deg=1)
                poly_tmp = Polynomial.cast(poly_tmp)
                cdelt1_approx = poly_tmp.coef[1]
                crval1_approx = poly_tmp.coef[0] - cdelt1_approx * crpix1
            if self.debugplot >= 10:
                print('\n>>> Approximate CRVAL1 :', crval1_approx)
                print('>>> Approximate CDELT1 :', cdelt1_approx)

            # compute differences with respect to the approximate linear
            # fit and start iterative removal of deviant points
            yfit = np.copy(wv_predicted)
            yfit -= crval1_approx + (xpos_predicted - crpix1) * cdelt1_approx
            poly_tmp, yres_tmp, reject = polfit_residuals_with_sigma_rejection(
                x=xpos_predicted, y=yfit, deg=deg_eff,
                times_sigma_reject=times_sigma_reject,
                color=color_symbol,
                xlim=(1, NAXIS1_EMIR),
                xlabel="xpos_predicted [from 1 to NAXIS1]",
                ylabel="differences with\nlinear solution (Angstrom)",
                title="Estimating wavelength calibration polynomial",
                use_r=False,
                debugplot=self.debugplot
            )
        else:
            reject = np.zeros(nlines, dtype=np.bool)

        # compute polynomial fit to final data set
        xfit = xpos_predicted[np.logical_not(reject)]
        yfit = wv_predicted[np.logical_not(reject)]
        poly_wv = Polynomial.fit(x=xfit, y=yfit, deg=deg_eff)
        poly_wv = Polynomial.cast(poly_wv)
        if self.debugplot >= 10:
            print("\n>>> Polynomial fit:\n" + str(poly_wv))
